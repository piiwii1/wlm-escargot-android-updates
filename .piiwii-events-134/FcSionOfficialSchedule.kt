...
object FcSionOfficialSchedule {
    ...
    private const val BASE_MATCH_DURATION_MS = 2L * 60L * 60L * 1000L
    ...
    fun sync(context: Context): SyncResult {
        ...
        val postMatchMs = config.settings.fcSionPostMatchMinutes.coerceIn(0, 360) * 60L * 1000L
        val activeWindowMs = BASE_MATCH_DURATION_MS + postMatchMs
        val primary = runCatching { fetchFootballCh(activeWindowMs) }
        val fallback = if (primary.isFailure) runCatching { fetchFcSionHome(activeWindowMs) } else null
        ...
        val end = fixture.kickoffMs + activeWindowMs
        val now = System.currentTimeMillis()
    ...
    private fun fetchFootballCh(activeWindowMs: Long): Fixture {
    ...
    private fun fetchFcSionHome(activeWindowMs: Long): Fixture {
        ...
        val now = System.currentTimeMillis()
        if (kickoff + activeWindowMs <= now) error("match affiché déjà terminé")
