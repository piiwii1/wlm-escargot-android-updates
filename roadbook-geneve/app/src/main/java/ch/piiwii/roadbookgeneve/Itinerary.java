...
public final class Itinerary {
    ...
    public static final String TOMATO_DESTINATION = AROMAT_DESTINATION;
    ...
    public static final Stop[] STOPS = new Stop[] {
            ...
            new Stop("geneva_rp", "18:00", "Geneva Vice / Geneva RP",
                    ...
                    false, 0, true),
            new Stop("leave_village", "20:45", "Départ pour Rêve d'Eau",
                    "Marge pour circulation, parking au centre et marche jusqu'au Jardin Anglais.",
                    null, null, true, 15, false),
            new Stop("rock", "22:00", "Rêve d'Eau — Rock & Pop",
                    ...
                    true, 20, true),
            new Stop("choice", "22:20", "Choix pour la suite",
    ...
    public static void setTime(Context context, Stop stop, int hour, int minute) {
        prefs(context).edit().putString("time_" + stop.id,
                String.format(java.util.Locale.ROOT, "%02d:%02d", hour, minute)).apply();
    }

    public static long getTimeMillis(Context context, Stop stop) {
        String[] parts = getTime(context, stop).split(":");
        LocalDateTime local = DAY.atTime(Integer.parseInt(parts[0]), Integer.parseInt(parts[1]));
